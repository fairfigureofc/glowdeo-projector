...
data class Corner(
    ...
    fun isInside(): Boolean = x.isFinite() && y.isFinite() && x in 0f..1f && y in 0f..1f
}
...
data class Quad(
    ...
    fun isValid(): Boolean {
        if (points.size != 4 || points.any { !it.isInside() }) return false
        val crosses =
    ...
    companion object {
        fun rectangle(
            ...
            right: Float,
            bottom: Float,
        ): Quad = Quad(listOf(Corner(left, top), Corner(right, top), Corner(right, bottom), Corner(left, bottom)))

        fun decode(
            text: String?,
            fallback: Quad,
        ): Quad {
            val tokens = text?.split(',').orEmpty()
            val values = tokens.mapNotNull { it.toFloatOrNull() }
            return if (tokens.size == 8 && values.size == 8) {
                Quad(values.chunked(2).map { Corner(it[0], it[1]) }).takeIf { it.isValid() } ?: fallback
            } else {
                fallback
            }
        }
